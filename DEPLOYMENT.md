...
| URL | Meaning |
|---|---|
| `.../reqmesh/v0.1.31/scripts/install.sh` | that release — reproducible, recommended |
...
```bash
curl -fsSL https://raw.githubusercontent.com/CallumNunesVaz/reqmesh/v0.1.31/scripts/install.sh | bash
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/CallumNunesVaz/reqmesh/v0.1.31/scripts/install.sh \
...
```bash
tar xzf reqmesh-v0.1.31.tar.gz && cd reqmesh-v0.1.31
...
```bash
docker load < reqmesh-v0.1.31-image.tar.gz
