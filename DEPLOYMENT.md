...
| URL | Meaning |
|---|---|
| `.../reqmesh/v0.1.24/scripts/install.sh` | that release — reproducible, recommended |
...
```bash
curl -fsSL https://raw.githubusercontent.com/CallumNunesVaz/reqmesh/v0.1.24/scripts/install.sh | bash
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/CallumNunesVaz/reqmesh/v0.1.24/scripts/install.sh \
...
```bash
tar xzf reqmesh-v0.1.24.tar.gz && cd reqmesh-v0.1.24
...
```bash
docker load < reqmesh-v0.1.24-image.tar.gz
