...
| URL | Meaning |
|---|---|
| `.../reqmesh/v0.1.12/scripts/install.sh` | that release — reproducible, recommended |
...
```bash
curl -fsSL https://raw.githubusercontent.com/CallumNunesVaz/reqmesh/v0.1.12/scripts/install.sh | bash
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/CallumNunesVaz/reqmesh/v0.1.12/scripts/install.sh \
...
```bash
tar xzf reqmesh-v0.1.12.tar.gz && cd reqmesh-v0.1.12
...
```bash
docker load < reqmesh-v0.1.12-image.tar.gz
