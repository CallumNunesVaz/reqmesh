...
def _commit_due(schedule: str, *, count: int, interval_hours: float,
...
def _git_schedule_for(git_cfg: dict) -> tuple[str, float, int]:
    return (
...
async def _commit_project(project_id: str, project_root: Path, msg: str, username: str = "") -> bool:
        ...
        try:
            ...
            if not force:
                try:
                    ...
                    git_cfg = {}
                schedule, interval_hours, threshold = _git_schedule_for(git_cfg)
                if not _commit_due(
                    schedule,
            ...
            if await _commit_project(project_id, project_root, msg):
...
async def _git_flush_loop() -> None:
    while True:
        try:
            ...
            logging.getLogger(__name__).exception("git flush loop iteration failed")
