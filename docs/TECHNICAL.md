# reqmesh — technical guide

Everything below the surface: how the tool is put together, how to run it from
source, how authentication and permissions resolve, what each page does in
detail, and how the data is laid out on disk. The [README](../README.md) is
the short version; [DEPLOYMENT.md](../DEPLOYMENT.md) covers production installs
and every `RT_*` setting; [api.md](api.md) is the generated route reference.

The screenshots are the bundled **Cessna 172S Skyhawk SP** example — 57
requirements with real parametrics, a full physical breakdown and a populated
risk register — which ships with the app and seeds on first run.
...
- [The pages](#the-pages)
- [Cross-linking, selection and guidance](#cross-linking-selection-and-guidance)
...
pytest, covering the API, storage, auth, integrity, quality, tracing, code scan, fingerprint, table I/O, evaluation, what-if impact, and deployment. A separate, slower OpenAPI-generated contract suite is run apart from the default (see `ci.yml`):
...
vitest unit tests covering stores, API client, entities,
...
![The Users page: accounts, roles, status and per-row actions](screenshots/users.png)
...
![Git integration in project settings: commit author, remote URL and the auto-commit schedule](screenshots/git-panel.png)
...
One section per sidebar entry, in sidebar order where the page is its own
thing; the requirement-level features (parametrics, coverage, review, quality)
follow the pages that surface them.
...
![The requirements workspace: tree, graph canvas and inspector](screenshots/requirements.png)
...
![A requirement's parameters, constraints and measured verdicts](screenshots/requirement-detail.png)
...
![The traceability matrix](screenshots/traces.png)
...
![The verification cases page](screenshots/verification.png)
...
![Project metrics: coverage, gaps, quality scores, traceability and the risk profile](screenshots/metrics.png)
...
![The allocation matrix: requirements against components, click a cell to allocate](screenshots/allocation.png)
...
![Architecture decision records](screenshots/decisions.png)
...
![Reusable constraint and calc definitions](screenshots/definitions.png)
...
## Cross-linking, selection and guidance
...
The bulk actions themselves are exposed as API endpoints — see the
[bulk operations](api.md#bulk-operations) section of the API reference.
...
Production installs — TLS, the reverse proxy, email, git push and air-gapped
mode — are covered end to end in **[DEPLOYMENT.md](../DEPLOYMENT.md)**, and the
one-line installer is quoted in the [README](../README.md#get-started) (the
release process rewrites the tag it pins, so it is deliberately not repeated
here).

Raw `docker compose` works if you prefer to wire it up yourself:
...
The complete API reference — every route the app registers, with its method and
description — is generated from the OpenAPI schema and lives in
**[docs/api.md](api.md)**. It is generated rather than hand-written, so it
cannot drift from the code: `backend/gen_api_reference.py` renders it from the
live route table, and CI regenerates and diffs the file on every push.

When adding or changing a route, give it a `summary=` on the decorator (that is
the row's description in the reference) and run
`backend/.venv/bin/python backend/gen_api_reference.py`.
