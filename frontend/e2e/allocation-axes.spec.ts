...
for (const axis of AXES) {
  test(`the ${axis.tab} matrix shows its own relationship`, async ({ app, server }) => {
    ...
    await expect(app.locator('main')).toContainText(`Requirements × ${axis.label}`, { timeout: 10_000 });
...
test('a cell on the risks matrix writes the risk, and survives a reload', async ({ app, server }) => {
  ...
  await expect(app.locator('main')).toContainText('Requirements × Risks', { timeout: 10_000 });
  ...
  await expect(async () => {
    ...
    expect(after.rows.find((r3: any) => r3.req_id === reqId).cells[riskId]).toBe(true);
  }).toPass({ timeout: 10_000 });
});
...
test('the Baselines tab appears and switches the matrix', async ({ app, server }) => {
  ...
  await tab.click();
  // The matrix must be for baselines — the expect has its own timeout.
  await expect(app.locator('main')).toContainText('Requirements × Baselines', { timeout: 10_000 });
});
...
test('baseline columns render in sequence order, not alphabetically', async ({ app, server }) => {
  ...
  // Wait for the matrix to switch — the expect has its own timeout.
  await expect(app.locator('thead')).toContainText('SRR', { timeout: 10_000 });
...
test('a due date appears in the column header', async ({ app, server }) => {
  ...
  // The expect has its own timeout, no fixed sleep needed.
  await expect(app.locator('thead')).toContainText('2026-01-01', { timeout: 10_000 });
...
test('toggling a baseline cell persists across a reload', async ({ app, server }) => {
  ...
  // Poll the API until the cell toggle lands — no fixed sleep.
  await expect(async () => {
    const live = await api<any>(app, `/projects/${P}/allocation-matrix?axis=baselines`);
    const row2 = live.rows.find((r2: any) => r2.req_id === reqId);
    expect(row2?.cells[colId]).toBe(true);
  }).toPass({ timeout: 10_000 });
  ...
  await expect(async () => {
    ...
    const updatedRow = after.rows.find((r3: any) => r3.req_id === reqId);
...
test('toggling one baseline does not tick other cells in that row', async ({ app, server }) => {
  ...
  await expect(async () => {
    const live = await api<any>(app, `/projects/${P}/allocation-matrix?axis=baselines`);
    const r2 = live.rows.find((r2: any) => r2.req_id === row.req_id);
    expect(r2?.cells[colId]).toBe(true);
  }).toPass({ timeout: 10_000 });
  ...
  await expect(async () => {
    const after = await api<any>(app, `/projects/${P}/allocation-matrix?axis=baselines`);
    const updatedRow = after.rows.find((r3: any) => r3.req_id === row.req_id);
    expect(updatedRow.cells[colId]).toBe(true);
...
test('moving a baseline down reorders columns on the matrix', async ({ app, server }) => {
  ...
  await app.waitForSelector('main');
  await setEditMode(app, true);
  await app.waitForTimeout(1000); // let edit-mode controls settle
  ...
  await expect(async () => {
    const mid = await api<any>(app, `/projects/${P}/allocation-matrix?axis=baselines`);
    expect(ours(mid)).toEqual(['ZZA', 'ZZC', 'ZZB']);
  }).toPass({ timeout: 10_000 });
...
test('a backwards due date shows the server error message', async ({ app, server }) => {
  ...
  await app.waitForTimeout(500); // let the edit dialog animate in
  await app.locator('input[type="date"]').fill('2026-01-01');
  await app.waitForTimeout(300); // let the input settle
  await app.getByRole('button', { name: 'Save' }).click();
  // The error surfaces as a toast — the expect below has its own timeout.
  await expect(app.locator('body')).toContainText('Due dates must not go backwards', { timeout: 10_000 });
});
