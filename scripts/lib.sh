...
# Every global IPv4 address this box answers on, newline-separated.
#
# `detect_lan_ip` returns one address — whatever `ip route get` calls the
# preferred source — and a host with more than one is not unusual: a NIC
# carrying a primary and a secondary lease, or a wired and a wireless
# interface. Naming only one in the Caddy site block means a request to any
# *other* local address matches no site, and Caddy answers 200 with an empty
# body: a blank page, no error, and a `curl https://localhost/` that looks
# perfectly healthy. That is exactly what happened on a reinstall where
# detection picked the secondary of two addresses on one NIC.
#
# Docker's own bridges are excluded by interface name. They are global-scope
# addresses too, but nothing browses to them and asking Caddy to hold a
# certificate for each is noise.
detect_lan_ips() {
    has_cmd ip || { detect_lan_ip; return; }
    ip -4 -o addr show scope global 2>/dev/null | awk '
        $2 ~ /^(docker|br-|veth|virbr|lo)/ { next }
        {
            split($4, a, "/")
            if (a[1] != "" && !seen[a[1]]++) print a[1]
        }'
}

# The host part of a base URL, or empty. Used to make sure the address the
# operator is told to browse to is one the proxy actually serves.
base_url_host() {
    local url="${1:-}"
    [ -n "$url" ] || return 0
    url="${url#*://}"
...
#
# Every local address is named, not just the detected one, so a multi-homed
# host serves on all of them — and so does a host whose DHCP lease moves. The
# base URL's host leads the list when it is an address at all, because that is
# the address the installer tells the operator to browse to and the one its own
# post-deploy check uses; if the two disagree the deployment looks broken while
# every component is healthy.
_caddy_lan_addresses() {
    local addrs="" seen=" " ip
    local base_host; base_host="$(base_url_host "${CFG[BASE_URL]:-}")"

    # The base-URL host first, when it is not a name Caddy already covers.
    case "$base_host" in
        ""|localhost|127.0.0.1) ;;
        *) addrs="https://$base_host"; seen=" $base_host " ;;
    esac

    for ip in $(_caddy_known_ips); do
    ...
    [ -n "$addrs" ] && addrs+=", "
    addrs+="https://localhost, https://127.0.0.1"
    printf '%s' "$addrs"
}

# The detected addresses, preferring an explicit LAN_IP so a caller (and the
# tests) can pin them without a live network.
_caddy_known_ips() {
    if [ -n "${CFG[LAN_IPS]:-}" ]; then
        printf '%s\n' ${CFG[LAN_IPS]}
    elif [ -n "${CFG[LAN_IP]:-}" ]; then
        printf '%s\n' "${CFG[LAN_IP]}"
    fi
}
...
render_caddyfile() {
    ...
    #
    # It follows the base URL's host when there is one: that is the address the
    # operator browses to, and pinning the fallback to a *different* local
    # address is how a multi-homed box ends up serving an empty 200 on the only
    # address anybody uses.
    # Caddy requires the global options block first in the file.
    local sni; sni="$(base_url_host "${CFG[BASE_URL]:-}")"
    case "$sni" in ""|localhost|127.0.0.1) sni="$lan" ;; esac
